...
(define eval-calls
  (lambda (cls left-value env) 
    (let ((args (eval-calls-args cls left-value env))
          (left-value-evaluated (if (simple-value? left-value)
        ...
        (if (empty? args)
          ...
          (if (procval? left-value-evaluated)
              ...
              (let ((result (encontrar left-value-evaluated args)))             ;AQUI VA LO DE ARREGLOS   
                (if (and (eqv? (length result) 1) (not(andmap list? result)))
                    (car result)
                    result)))))))
